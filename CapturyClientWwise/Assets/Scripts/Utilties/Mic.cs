...
public class Mic : MonoBehaviour {
    ...
    public class Device {
        ...
        /// to it changing
        /// </summary>
        public event Action<int, int, int, float[]> OnFrameCollected;
        /// <summary>
        ...
        internal void BroadcastFrame(int frameSequence, int channelCount, float[] pcm) {
            if (VolumeMultiplier != 1) {
                for (int i = 0; i < pcm.Length; i++)
                    pcm[i] *= VolumeMultiplier;
            }
            OnFrameCollected?.Invoke(frameSequence, SamplingFrequency, channelCount, pcm);
        }
    }
    ...
    public Mic() { }
    static Mic instance;
    public static Mic Instance => instance;
    /// <summary>
    ...
    {
        if (instance != null) return;
        instance = gameObjectToAttach.AddComponent<Mic>();
    }
    static void StartRecording(Device device) {
        StopRecording(device);
        var newClip = Microphone.Start(device.Name, true, 8, device.SamplingFrequency);
        if (newClip != null) {
            clips.Add(device, newClip);
            prevPositions.Add(device, 0);
            pcms.Add(device, new CircularBuffer<float>(3840, 0));
        }
    }
    ...
    static Dictionary<Device, CircularBuffer<float>> pcms = new Dictionary<Device, CircularBuffer<float>>();
    ...
    void Update() {
        foreach (var pair in clips) {
            ...
            while (pcm.CurrentSize >= frame.Length) {
                for (int i = 0; i < frame.Length; i++)
                    frame[i] = pcm.Read();
                device.BroadcastFrame(frameSeq, clip.channels, frame);
            }
            // Update the last position of this device
            prevPositions[device] = pos;
            //Update frame sequence id
            frameSeq++;
        }
    }
}
